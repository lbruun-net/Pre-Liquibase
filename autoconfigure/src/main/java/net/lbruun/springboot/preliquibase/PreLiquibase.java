...
public class PreLiquibase {
    ...
    private String getDbPlatformCodeFromDataSource() throws ResolveDbPlatformError {
        ...
        final String dbPlatformCodeCandidate = LiquibaseUtils.getLiquibaseDatabaseShortName(dataSource);
    ...
    private String resolveDbPlatformCode() {
        final String dbPlatformCodeFromProps = properties.getDbPlatformCode();
        return dbPlatformCodeFromProps == null ? getDbPlatformCodeFromDataSource() : dbPlatformCodeFromProps;
    }
    ...
    private List<Resource> getScripts(List<String> resources) {
        ...
        if (resources.size() == 1 && (resources.get(0).endsWith("/") || resources.get(0).endsWith("\\"))) {
            final String pathLoc = resources.get(0);
            final List<String> absSqlFileLocations = new ArrayList<>();
            absSqlFileLocations.add(pathLoc + dbPlatformCode + ".sql");
        ...
        for (final String location : locations) {
            for (final Resource resource : doGetResources(location)) {
                if (resource.exists()) {
                    ...
                    final String msg = format("Resource \"%s\" is invalid or cannot be found", location);
    ...
    private List<Resource> getFilteredResources(List<Resource> resources) {
        ...
        final PropertyPlaceholderHelper placeholderReplacer = new PropertyPlaceholderHelper(
            // error on unresolvable placeholders
        ...
        for (final Resource resource : resources) {
            try (InputStream in = resource.getInputStream()) {
                ...
                throw new SqlScriptReadError("Could not read SQL script file \"" + resource + "\" into memory", ex);
            } catch (final IllegalArgumentException ex) {
                throw new SqlScriptVarError("Could not replace variables in script file \"" + resource + "\"", ex);
            }
        }
        return newList;
    }
    ...
    private void runScripts() {
        ...
        if (resources.size() > 1) {
            logger.info("PreLiquibase: Executing SQL scripts : {}", resources);
        } else {
            logger.info("PreLiquibase: Executing SQL script : {}", resources.get(0));
        }
