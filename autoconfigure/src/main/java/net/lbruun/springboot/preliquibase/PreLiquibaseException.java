...
public abstract class PreLiquibaseException extends RuntimeException {
  ...
  /**
   * Thrown on certain {@link PreLiquibase} methods if the method is invoked <i>prior</i> to
   * {@link PreLiquibase#execute()}.
   */
  public static class UninitializedError extends PreLiquibaseException {

    public static UninitializedError DEFAULT =
        new UninitializedError("Method must not be invoked prior to execute()");
  ...
  /**
   * Thrown on I/O errors during the stage where placeholder values in SQL script are being replaced
   ...
   * {@link PreLiquibaseProperties#setSqlScriptReferences(java.util.List) explicitly specified} and
   * one or more of the references is invalid (file not found, incorrect specification of Spring
   * Resource reference, etc).
   */
