...
public class LiquibaseUtils {
     ...
     * <p>
     * The method invokes
     * {@code liquibase.database.DatabaseFactory#findCorrectDatabaseImplementation()}
     * and therefore requires Liquibase library on the classpath.
     *
     * <p>
     * The method works like this: The database is determined by connecting to it
     * and retrieving {@link java.sql.DatabaseMetaData} which is then matched
     * against known database platforms in Liquibase. If there's any error during
     * this operation then {@link PreLiquibaseException} will be thrown. If there
     * are no errors, but the type of database simply isn't supported by Liquibase
     * then {@code "unsupported"} is returned. If all goes well a Liquibase database
     * shortname is returned as per the list above. The connection used to determine
     * the database type is closed again before the method exits.
     *
        ...
        try {
            ...
            if (nonNull(database)) {
                try {
                    ...
                    throw new ResolveDbPlatformError("Error while closing connection for DataSource", ex);
                }
            } else if (nonNull(connection)) {
                try {
