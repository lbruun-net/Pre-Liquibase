...
public class PreLiquibaseProperties {
  ...
  Predicate<Resource> targetFile = res -> {
    try {
  ...
  Predicate<Resource> targetDirectory = res -> {
    try {
  ...
  Function<Resource, List<Resource>> loadFiles = res -> {
    try {
      return List.of(res.getFile().listFiles()).stream().map(File::toURI).map(URI::toString).map(resourceLoader::getResource).collect(toList());
    } catch (final IOException e) {
      e.printStackTrace();
    }
    return List.of();
   ...
   * for example {@code "file:/foo/bar/myscript1.sql,file:/foo/bar/myscript2.sql,classpath:foo/bar/"}.
  ...
  public void setSqlScriptReferences(List<Resource> sqlScriptReferences) {
    final List<Resource> files = sqlScriptReferences.stream().filter(targetFile).collect(toList());

    final List<Resource> directories = sqlScriptReferences.stream().filter(targetDirectory).map(loadFiles).flatMap(List::stream).collect(toList());

    final String platformCode = getDbPlatformCode();
    this.sqlScriptReferences = Stream.concat(files.stream(), directories.stream()).filter(res -> res.getFilename().contains(platformCode)).collect(toList());
  }
  ...
  public List<Resource> getScripts() {
    ...
    final Resource typedFallback = resourceLoader.getResource(format("classpath:preliquibase/%s.sql", getDbPlatformCode()));
