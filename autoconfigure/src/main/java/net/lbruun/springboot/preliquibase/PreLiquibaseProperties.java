...
public class PreLiquibaseProperties {
   ...
   *
   * <p>
   * The value is interpreted slightly differently depending on its content:
   * <ul>
   * <li>If the value is a Spring Resource textual reference which ends with {@code "/"}: In this
   * case, the reference is expected to be a folder reference where SQL scripts can be found. From
   * this folder: If a file named "{@link #setDbPlatformCode(java.lang.String)
   * DBPLATFORMCODE}{@code .sql}" (e.g. "{@code postgresql.sql}") exists then that will be used. If
   * no such file is found then a file named "{@code default.sql}" is used. If neither file is found
   * then no action will be taken, similarly to {@link #setEnabled(boolean) disabling} the module.
   * <br>
   * Example values:<br>
   ...
   * references to <i>specific</i> SQL files. Each script file will be executed in the order they
   * are listed. Before execution of any of of the script files it is checked if all files mentioned
   * in the list actually exists. If not, an {@link PreLiquibaseException.SqlScriptRefError}
   * exception is thrown. <br>
   * Example value: {@code "file:/foo/bar/myscript1.sql,file:/foo/bar/myscript2.sql"}.</li>
   * </ul>
   *
   * <p>
