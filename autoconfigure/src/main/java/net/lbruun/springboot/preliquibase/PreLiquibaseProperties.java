...
public class PreLiquibaseProperties {
    ...
    public List<Resource> getScripts() {
        ...
        List<Resource> directories = getSqlScriptReferences().stream().filter(targetDirectory).map(loadFiles).flatMap(List::stream).collect(toList());

        String platformCode = getDbPlatformCode();

        sqlScriptReferences = Stream.concat(files.stream(), directories.stream()).filter(res -> res.getFilename().contains(platformCode)).collect(toList());
        ...
        Resource typedFallback = resourceLoader.getResource(format("classpath:preliquibase/%s.sql", getDbPlatformCode()));
