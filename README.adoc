...
- Java 8 or later 
- Spring Boot 2.3 or later (see <<Spring Boot compatibility>>)
...
*_use either all lower-case or all upper-case for schema name, never mixed case_*. In short 'Foo_bar' is not a good value, 
...
Spring Boot 2.5 introduced changes to its auto-configuration classes for Liquibase which has necessitated two concurrent 
release branches of the Pre-Liquibase library. Be sure to use the version of Pre-Liqquibase appropriate for your
Spring Boot version:
...
|Legacy release supporting Spring Boot 2.3 and Spring Boot 2.4. This release branch will 
continue to receive bug fixes until Spring Boot 2.4 has https://github.com/spring-projects/spring-boot/wiki/Supported-Versions[has reached end-of-life]. 
However, new features will not be introduced on this branch.
...
In summary:  Version 1.1.x or later is the recommended version of Pre-Liquibase but it requires Spring Boot 2.5 or later.
